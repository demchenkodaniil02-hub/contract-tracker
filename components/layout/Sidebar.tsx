...
import { LayoutDashboard, FileText, Building2, Users, HardHat, LogOut } from 'lucide-react'
    ...
    <aside style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '4px 8px' }}>
        <div style={{
          ...
          background: 'linear-gradient(160deg, #2f6bdc, #1f4ba8)', color: '#fff', flexShrink: 0,
        }}>
          <HardHat size={22} />
        </div>
        <div>
      ...
      {/* Legend */}
      <div style={{
        marginTop: 'auto', padding: '14px 12px 4px',
        borderTop: '1px solid rgba(255,255,255,.08)',
        display: 'flex', flexDirection: 'column', gap: 9,
        fontSize: 12, color: '#8a96af',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ width: 9, height: 9, borderRadius: '50%', background: '#2f6bdc', display: 'inline-block' }} />
          МАФ / Металл
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ width: 9, height: 9, borderRadius: '50%', background: '#e07a1a', display: 'inline-block' }} />
          Отделка
        </div>
      </div>
      ...
      {/* Профиль */}
      {profile && (
        <Link href="/profile" style={{ margin: '4px 16px 0', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 9, textDecoration: 'none', background: pathname === '/profile' ? '#2f6bdc' : 'rgba(255,255,255,.05)', transition: 'background .15s' }}>
