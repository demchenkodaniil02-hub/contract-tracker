              ...
              {paginated.map((c) => {
                ...
                return (
                  <tr key={c.id} style={{ cursor: 'pointer', transition: 'background .12s' }}
                    ...
                    <td style={{ ...S.td, color: 'var(--muted-ink)', maxWidth: 180, whiteSpace: 'normal' }}>{customer?.name ?? '—'}</td>
                    <td style={{ ...S.td, color: 'var(--muted-ink)', maxWidth: 180, whiteSpace: 'normal' }}>{contractor?.name ?? '—'}</td>
