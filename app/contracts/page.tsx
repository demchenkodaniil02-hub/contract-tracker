              ...
              {paginated.map((c) => {
                ...
                return (
                  <tr key={c.id} style={{ cursor: 'pointer', transition: 'background .12s' }}
                    ...
                    <td style={{ ...S.td, color: 'var(--muted-ink)', maxWidth: 180 }}><div style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{customer?.name ?? '—'}</div></td>
                    <td style={{ ...S.td, color: 'var(--muted-ink)', maxWidth: 180 }}><div style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{contractor?.name ?? '—'}</div></td>
